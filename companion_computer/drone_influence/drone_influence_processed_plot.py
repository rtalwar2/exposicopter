...
# New subplot for average of "Median" per file
for file_name, avg_value in average_median_dbm_per_file.items():
    ...
    # Scatter plot point
    axes[2, 1].scatter(file_name, avg_value, color=color, marker=marker)
axes[2, 1].set_title("Average of 'Median' per CSV File (dBm)")
axes[2, 1].set_xlabel("CSV File")
axes[2, 1].set_ylabel("dBm")
axes[2, 1].tick_params(axis='x', rotation=45)  # Rotate x-axis labels for readability
